...
let Todo = (props) => {
  ...
  return (
    <div className={`todo ${props.taskCompleted === true ? 'completed' : ''}` }>
      <button className={`todo-checkbox ${props.taskCompleted === true ? 'completed' : ''}`} onClick={ taskComplete }></button>
      <p className='todo-task'>{ props.todoData }</p>
      <button className='remove-task-btn' onClick={ clickHandler }><CrossIcon /></button>
    </div>
  );
}
